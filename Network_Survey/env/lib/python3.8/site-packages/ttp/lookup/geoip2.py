...
try:
    ...
    log.error("ttp.lookup.geoip2: failed to import geoip2 library, install: pip install geoip2")
    HAS_LIBS = False
        ...
        for dbname, path_to_db in lookup_tag_data.items():
            if dbname.lower() in ret:
                ...
                try:
                    ...
                    log.error("ttp.lookup.geoip2: something went wrong, failed to load '{}' mmdb from '{}' path".format(
                        dbname, path_to_db
                        )
                    )
    return ret