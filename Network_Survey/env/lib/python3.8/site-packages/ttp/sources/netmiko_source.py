...
def netmiko_fun(input_name, **kwargs):
    ...
    # run commands
    log.info("TTP input netmiko_fun: sending - '{}', to - '{}'".format(commands, devices))
    for item in devices_dicts:
        output = ""
        net_connect = ConnectHandler(**item)
        for command in commands:
            log.info("TTP input netmiko_fun: sending - '{}', to - '{}'".format(command, item.get("host")))
            output += "\n" + net_connect.send_command(command, strip_prompt=False, strip_command=False)
