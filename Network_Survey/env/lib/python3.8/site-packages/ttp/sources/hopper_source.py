...
def hopper_fun(input_name, **kwargs):
    ...
    kwargs["credentials"].append((username, password,))
    log.info("TTP input hopper_fun: sending - '{}', to - '{}'".format(kwargs.get("commands"), kwargs.get("devices")))
    # get data from devices
    hopperObj = hopper()
    result = hopperObj.run(**kwargs)
    log.info("TTP input hopper_fun: received output from {} devices".format(len(result.keys())))
